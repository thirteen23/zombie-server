...
exports.seed = () => {
    ...
    fs.createReadStream(`${__dirname}/grades.csv`).pipe(csv({ objectMode: true, columns: true }))
        .on('data', (row) => {
            ...
            const product_group = row.product_group == '' ? null : row.product_group;
            const active = bool(row.active);
            rows++;
            pool.query('INSERT INTO web.grades(name, product_id, product_group, active) VALUES($1, $2, $3, $4)', [name, product_id, product_group, active],
                (err, res) => {
